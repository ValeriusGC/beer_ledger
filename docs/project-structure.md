...
**Последнее обновление:** 2026-09-24 16:51:11 +0300  
**Версия:** 6  
...
│           └── home/                     # UI Projection: Factory, Builder, HomePage
...
Контекст не импортирует `domain/` и `infrastructure/` соседа. Домен журнала не импортирует `Clicker`; из домена порции журнальный домен берёт только `ClickerId` (ссылка на чужой агрегат по id). Перевод осей живой порции на язык журнала — `axisRecordInputsFrom` в `journal/application`. `Click.record` принимает `List<AxisRecordInput>` и по-прежнему резолвит единицу и считает вклад в базовой единице.
...
Итерация 3.5 закрыта: главная собрана по UI Projection в `journal/presentation/home/`. Дальше — итерация 4.
